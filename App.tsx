...
// Add OneSignal to window object for TypeScript
declare global {
  interface Window {
    OneSignal: any;
  }
...
const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            if (session?.user) {
                // OneSignal Integration: Set External User ID
                if (window.OneSignal) {
                ...
                // OneSignal Integration: Remove External User ID on logout
                if (window.OneSignal) {
                    window.OneSignal.push(() => {
        ...
        if (session?.user) {
             channel = supabase
                ...
                .on(
                    ...
                    (payload) => {
                        addToast(`Thông báo mới: ${payload.new.message}`, 'info');
    ...
    const createNotification = useCallback(async (notification: Omit<Notification, 'id' | 'created_at' | 'read'>) => {
        // 1. Insert into DB for in-app notification
        const { error } = await supabase.from('notifications').insert([{ ...notification, read: false }]);
        if (error) console.error('Error creating DB notification:', error.message);
        ...
        try {
            ...
            // Optionally show a toast to the admin that push failed
        }
    }, []);
    ...
    const notifyAdmins = useCallback(async (message: string, link: string) => {
        ...
        // 1. Insert into DB for in-app notifications
        const notificationsToInsert = adminIds.map(id => ({
            user_id: id,
            message,
            link,
            read: false,
        ...
        try {
            ...
            console.error('Error sending push notifications to admins:', err.message);
        }
    }, []);
